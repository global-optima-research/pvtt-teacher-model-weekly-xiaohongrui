...
我们分别在 **1.3B** 和 **14B** 两个模型规模上，采用 **7 种不同的输入策略** 进行了系统性测试。
...
| 实验8 | ComfyUI 1:1 复刻（去背景+LoRA+单步） | 可替换，形状/一致性差 *(LoRA 失败，退化为实验7变体)* | 可替换，形状/一致性差 *(LoRA 成功，完整复刻)* |

> 标注 **加粗** 的为 14B 与 1.3B 结果不同的关键差异。
...
3. **ComfyUI 工作流不具有普遍性**：ComfyUI 工作流或许在某些特定样本上能较好地实现目标替换，但在本次测试的茶壶→橡皮鸭场景中，即使完整复刻了所有预处理步骤（参考图去背景、中性色填充、GrowMask+BlockifyMask）和采样参数（CFG=6.0, CFG_Star=5.0, LightX2V LoRA 单步推理），主体一致性保持依然非常差。**VACE 无法 zero-shot 满足 PVTT 项目要求，需要对模型进行改造，重点是保持主体一致性。**

4. **多视角参考图的潜在需求**：当前实验仅使用单张正面参考图，但原始视频中物体可能涉及旋转（如本次茶壶示例）。单视角参考图很难为模型提供足够的 3D 外观信息来应对多角度场景。**是否需要输入多张不同角度的参考图？如果需要，模型架构是否需要进行相应调整来接受多张 ref image？** 这是后续模型改造需要考虑的关键问题。

5. **主体一致性是核心瓶颈**：在所有 8 组实验中，所有能够实现替换的实验，生成的目标物体在帧间的外观一致性都很差，无法维持稳定的商品外观。这不是输入策略或采样参数能解决的问题，而是 VACE 模型架构在 subject-driven generation 上的根本局限。

### 对 PVTT 项目的结论

**Wan2.1-VACE 模型（1.3B 和 14B）在当前架构下无法 zero-shot 满足 PVTT 商品视频替换的项目要求**，具体问题包括：

- **形状保真度不足**：生成物体形状受原物体 mask 轮廓的强烈影响，无法真实还原参考图像中商品的正确形状
- **主体一致性差**：替换后的商品在视频帧间外观不一致，无法维持稳定的商品视觉呈现
- **替换控制性弱**：模型对参考图像的遵循度低，更倾向于 inpainting 而非 subject-driven generation
- **单视角参考图局限**：对于涉及物体旋转的场景，单张参考图无法提供足够的外观信息

### 后续方向

1. **模型改造**：VACE 需要增强主体一致性保持能力，可能需要引入 subject-driven generation 机制（如 IP-Adapter、ReferenceNet 等），或在训练阶段加入主体一致性损失
2. **多视角参考图支持**：探索模型架构调整以支持多张不同角度的参考图输入，提供更完整的 3D 外观信息
3. **两阶段流程**：考虑先用专业的 subject-driven 图像生成模型生成单帧替换结果，再用视频生成/编辑模型保持时间一致性
4. **替代方案评估**：评估其他具有更强 subject identity preservation 能力的模型或方法
