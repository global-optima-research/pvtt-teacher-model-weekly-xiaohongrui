"""
PVTT 数据集批量推理：Canny 首帧粘贴 + Bbox Mask
...
2. 对每个任务：
   ...
   d. 从 RGBA 参考图生成 Canny 边缘
   e. 从精确 mask 生成 Bbox mask（带 margin 扩展）
   f. 首帧 mask 区域嵌入白色 Canny 线条 + 中性色填充
   g. VACE 推理（ref image + canny-embedded frames + bbox mask + prompt）
   h. bbox mask 合成 + 保存结果

与 run_pvtt_canny_paste.py 的区别：使用 Bbox mask 代替 GrowMask + BlockifyMask。

输出结构（每个任务）：
    {save_dir}/
    ├── canny_paste_bbox.mp4                   # 合成后的目标视频
    ├── canny_paste_bbox_comparison.jpg        # [原视频首帧|mask video首帧|ref img|target首帧] + prompt
    ├── canny_paste_bbox_showcase.jpg          # [target首帧|target尾帧]
    ├── ref_canny.png                          # 参考图 Canny 边缘图
    └── experiment.log

用法：
    python run_pvtt_canny_paste_bbox.py \\
        --dataset_root ../../samples/pvtt_evaluation_datasets \\
        --output_root ../../experiments/results/1.3B/pvtt_canny_paste_bbox \\
        --model_size 1.3B
"""
...
def save_four_panel_comparison(output_dir, orig_first, vace_input_first,
    ...
    labels = ["Source Video", "VACE Input (Canny+Bbox)", "Reference Image", "Target Output"]
    ...
    path = output_dir / "canny_paste_bbox_comparison.jpg"
...
def save_showcase(output_dir, name, comp_frames, logger):
    ...
    first = Image.fromarray(comp_frames[0]) if isinstance(comp_frames[0], np.ndarray) \
        else comp_frames[0]
    last = Image.fromarray(comp_frames[-1]) if isinstance(comp_frames[-1], np.ndarray) \
        else comp_frames[-1]
    w, h = first.size
...
def process_single_task(entry, pipe, pipe_base_kwargs_template, args,
    ...
    try:
        ...
        video_path = output_dir / "canny_paste_bbox.mp4"
        ...
        # 首末帧展示
        save_showcase(output_dir, "canny_paste_bbox", composited, logger)
...
def main():
    parser = argparse.ArgumentParser(
        description="PVTT 数据集批量推理：Canny 首帧粘贴 + Bbox Mask")
    ...
    logger = setup_logger("pvtt_canny_paste_bbox", output_root)

    logger.info("=" * 60)
    logger.info("PVTT 批量推理：Canny 首帧粘贴 + Bbox Mask")
    logger.info("=" * 60)
    ...
    for idx, entry in enumerate(entries):
        ...
        if args.skip_existing:
            existing_video = output_root / save_dir / "canny_paste_bbox.mp4"
