"""
PVTT 数据集批量推理：中性色填充 + Bbox Mask（原始输出，不合成）

与 run_pvtt_neutral_fill_bbox.py 的唯一区别：
  输出视频直接是模型生成的原始视频，不经过 composite_with_mask 合成。
  用于验证"割裂感"是否来自合成步骤。

输出结构（每个任务）：
    {save_dir}/
    ├── neutral_fill_bbox_raw.mp4                # 模型直接生成的视频（无合成）
    ├── neutral_fill_bbox_raw_comparison.jpg     # [原始首帧|mask video首帧|ref img|target首帧] + prompt
    ├── neutral_fill_bbox_raw_showcase.jpg       # [target首帧|target尾帧]
    └── experiment.log

用法：
    python run_pvtt_neutral_fill_bbox_raw.py \\
        --dataset_root ../../samples/pvtt_evaluation_datasets \\
        --output_root ../../experiments/results/1.3B/pvtt_neutral_fill_bbox_raw \\
        --model_size 1.3B
"""
...
def save_four_panel_comparison(output_dir, orig_first, filled_first,
                                ...
                                prompt, logger):
    """[原始首帧 | mask video首帧 | ref img(VACE输入) | target首帧] 四列对比 + prompt。"""
    ...
    labels = ["Source Video", "Mask Video (Fill)", "Reference Image", "Raw Output (No Comp)"]
    ...
    path = output_dir / "neutral_fill_bbox_raw_comparison.jpg"
...
def process_single_task(entry, pipe, pipe_base_kwargs_template, args,
    ...
    try:
        ...
        # 7. 直接保存模型原始输出（不做 composite_with_mask 合成）
        video_path = output_dir / "neutral_fill_bbox_raw.mp4"
        ...
        # 首末帧展示
        save_showcase(output_dir, "neutral_fill_bbox_raw", generated, logger)

        del video_data, generated, filled_frames
...
def main():
    parser = argparse.ArgumentParser(
        description="PVTT 数据集批量推理：中性色填充 + Bbox Mask（原始输出，不合成）")
    ...
    logger = setup_logger("pvtt_neutral_fill_bbox_raw", output_root)

    logger.info("=" * 60)
    logger.info("PVTT 批量推理：中性色填充 + Bbox Mask（原始输出，不合成）")
    logger.info("=" * 60)
    ...
    for idx, entry in enumerate(entries):
        ...
        if args.skip_existing:
            existing_video = output_root / save_dir / "neutral_fill_bbox_raw.mp4"
