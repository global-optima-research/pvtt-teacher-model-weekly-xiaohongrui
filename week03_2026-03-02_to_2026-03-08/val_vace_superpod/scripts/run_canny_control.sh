#!/bin/bash
# =============================================================================
# 双参考图（商品图 + Canny 结构图）+ 中性色填充 + GrowMask + BlockifyMask 商品替换实验
#
# 对应 Python: baseline/wan2.1-vace/exp_canny_control.py
#
# 给 VACE 同时提供两张参考图：商品外观图 + 该商品的 Canny 边缘图，
# 输入视频帧保持标准中性色填充（不在帧内嵌入 Canny），
# 结构信息完全通过参考图传递。
#
# 所有参数均可通过环境变量覆盖：
#   CANNY_LOW=80 CANNY_HIGH=180 bash scripts/run_canny_control.sh
# =============================================================================
set -e
...
OUTPUT_DIR="${OUTPUT_DIR:-$PROJECT_DIR/experiments/results/${MODEL_SIZE}/canny_control/${TIMESTAMP}}"
...
# ---- 打印配置 ----
echo "============================================"
echo "双参考图（商品图 + Canny 结构图）商品替换实验"
echo "============================================"
echo "模型:            $MODEL_SIZE"
echo "样本:            $SAMPLE_NAME"
...
python3 "$BASELINE_DIR/exp_canny_control.py" \
